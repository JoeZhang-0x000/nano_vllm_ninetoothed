...
def arrangement(input, weight, output, eps,
...
@lru_cache(1)
def premake(ndim):
    kernel = make(arrangement, application, (Tensor(ndim), Tensor(ndim), Tensor(ndim), Tensor(0)), max_num_configs=MAX_NUM_CONFIG)
    return kernel


@register_ninetoothed_op
def rms_forward(input, weight, eps, inplace=False):
    assert weight.dim() == 1
    ndim = input.dim()
